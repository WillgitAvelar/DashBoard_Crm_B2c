...
function extractHotelName(produto) {
  if (!produto) return '-';
  let cleaned = produto.replace(/^(RES\d+-\d+\s*-\s*)?(CANCELADO\s+|EXPIRADO\s+|CONFIRMADO\s+)?/i, '');
  return cleaned.trim() || produto;
}

function extractStatus(produto) {
  if (!produto) return 'ATIVO';
  if (produto.includes('CANCELADO')) return 'CANCELADO';
  if (produto.includes('EXPIRADO')) return 'EXPIRADO';
  if (produto.includes('CONFIRMADO')) return 'CONFIRMADO';
  return 'ATIVO';
}
...
const resp = await fetch(`/api/b2c?${params}`);
  ...
  // Busca métricas
  const metricsResp = await fetch(`/api/b2c/metrics?${params}`);
  if (!metricsResp.ok) throw new Error('Erro ao buscar métricas B2C');
...
function updateB2cCharts(b2cData, metrics) {
  ...
  if (topHoteisCanvas) {
    ...
    const labels = metrics.hoteis_mais_vendidos.map(h => h.nome_hotel);
    const dataVals = metrics.hoteis_mais_vendidos.map(h => h.valor_total);
  ...
  if (statusPagamentoCanvas) {
    ...
    const labels = metrics.status_pagamento.map(s => s.status_pagamento);
    const dataVals = metrics.status_pagamento.map(s => s.quantidade);
...
async function handleB2cSubmit(e) {
  ...
  const b2cData = { data: data.data, nome_hotel: data.nome_hotel, valor: parseFloat(data.valor), status: extractStatus(data.nome_hotel), status_pagamento: data.status_pagamento };
  try {
    showLoading(true);
    ...
    if (!resp.ok) {
      ...
      const msg = errData?.error || 'Erro ao salvar B2C';
      throw new Error(msg);
    }
    closeModal('b2c');
    loadData();
